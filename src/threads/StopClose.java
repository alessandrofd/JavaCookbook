...
public class StopClose extends Thread {
    ...
    public void run() {
        try {
            ...
            // The following line will deadlock (intentionally), since HTTP enjoins the client to send a request
            // (like "GET / HTTP/1.0") and a null line, before reading the response.
            String line = is.readLine(); // DEADLOCK

            // Should only get out of the readLine if an interrupt is thrown, as a result of closing the socket.
